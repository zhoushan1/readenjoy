...
//post请求
    ...
    GetQueryString: (url, name)=> {
        ...
        var r = url.substring(url.indexOf("?") + 1, url.length).substr(1).match(reg);
        if (r != null) {
            return unescape(r[2]);
        }
        else {
...
//生成签名信息
    createSign:(timestamp,data={},uid=0,token=0)=>{

        var uinfo = wx.getStorageSync('uinfo')
        if(uinfo){
            uid = uinfo.meta.userInfo.id;
            token = uinfo.meta.token
        }else{
            uid = 0
            token = 0
        }
        var querystring = 'ai=look&ts='+timestamp+'&si=db2ff1e34eaccca2e9ca0e96222df2ee&di=604ebf4ed70fa9160269b813d5f1fca7&cv=1.1&os=ios&ov=10.0.2&nt=wifi&fm=json&cb=&vn=1.0&sc=667*375&ch=QD_AppStore&pn=com.culiu.look&im=DD5B6EED-D392-4F19-90F6-BBD4DA044D51&pt=&ge=0';
        querystring += '&uid='+uid;
        if(token){
        ...
        for(var j=0;j<sortArr.length;j++){
            new_querystr += sortArr[j] +'='+ params[sortArr[j]]
        }
        new_querystr += '9a6022bab7998800c8d5028a5a0a4487';
        //console.log(new_querystr)
        var sign = Util.MD5(new_querystr).toLowerCase();
        //console.log(sign)
        return sign;
